    ...
    : QWidget{parent}
{
    ...
    tile = new QTile(this);
    tile->initTile();
    connect(fightTimer, &QTimer::timeout, this, &QFight::fightOnTimer);
    connect(scenceTimer, &QTimer::timeout, this, &QFight::scenceOnTimer);
    fightTimer->start(50);
}

void QFight::load(IDTILE idTile[], NPCINFO npcInfo, ROLEINFO roleInfo)
{
...
void QFight::paintEvent(QPaintEvent *event)
{
    ...
    tile->draw(painter,rect.left() + TILE_WIDTH, rect.top() + TILE_HEIGHT / 2, 173);//画左上角那个主角标
    ...
    //主角
    //写字
    painter.drawText(rect.left() + 2.5 * TILE_WIDTH, rect.top() + TILE_HEIGHT * 1.5, "勇士");
